...
function search(matchValue) {
    var searchStr = matchValue || document.querySelector("#searchItem").value, cellList = document.querySelectorAll(".content .table > .table-row-group > .table-row > .table-cell");
...
function searchClue(searchStr) {
    var searchStrArray = [searchStr], clueCellList = document.querySelectorAll(".content .table.clue > .table-row-group > .table-row > .table-cell.clue-cell");
    for (var cu = 0; cu < clueCellList.length; cu++) {
        var $currentClueCell = clueCellList[cu], clueHTML = $currentClueCell.innerHTML;
...
function displayCell(searchStrArray, cellList) {
    for (var i = 0; i < cellList.length; i++) {
        var $currentCell = cellList[i], innerHTML = $currentCell.innerHTML, matchResult = indexOfArray(innerHTML, searchStrArray);
        if (matchResult.result) {
            $currentCell.classList.remove("hide");
            for (var ma = 0; ma < matchResult.matchArray.length; ma++) {
                var matchStr = matchResult.matchArray[ma], lightHtml = "<span class='light'>" + matchStr + "</span>";
                $currentCell.innerHTML = innerHTML.split(matchStr).join(lightHtml)
            }
...
function displayChapter() {
    ...
    for (var t = 0; t < tableList.length; t++) {
        var $currentTable = tableList[t], groupList = $currentTable.querySelectorAll(".table-row-group");
        for (var g = 0; g < groupList.length; g++) {
            var $currentGroup = groupList[g], rowList = $currentGroup.querySelectorAll(".table-row");
            ...
            if ($currentGroup.querySelectorAll(".table-row:not(.hide)").length === 0) {
                ...
                var $currentGroupHeader = $currentGroup.querySelector(".table-row.chapter-header"), $currentGroupContent = $currentGroup.querySelectorAll(".table-row:not(.chapter-header)");
...
function indexOfArray(str, array) {
    var matchResult = {"result": false, "matchArray": []};
